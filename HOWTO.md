...
6. Review all proposals (explicitly unresolved with a reason is allowed), enter your name and confirm the assessment/source checks, then **Sign off** that draft version. Download it as a signed PDF. **Prepare new brief version** always creates a further unsigned draft; it does not re-sign or replace the signed version. Neither signing nor downloading distributes the brief.
