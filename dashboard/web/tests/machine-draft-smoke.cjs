...
(async () => {
  ...
  try {
    ...
    const prepare = page.getByRole("button", { name: /^Prepare (new brief version|intelligence brief)$/ });
    await prepare.click();
    ...
    await page.getByRole("heading", { name: "Draft brief ready for sign-off", exact: true }).waitFor();
    await page.getByText("All versions (2)", { exact: true }).evaluate(el => { el.parentElement.open = true; });
    page.once("dialog", d => d.accept());
    await page.getByRole("button", { name: "Remove version 2", exact: true }).click();
    await page.getByRole("heading", { name: "Brief signed off", exact: true }).waitFor();
    await page.getByText("Assessment maintenance", { exact: true }).click();
    page.once("dialog", d => d.accept());
