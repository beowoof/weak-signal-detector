...
export default function FollowupCollection({ scenario, noticeId, reviewVersion, onComplete }) {
  ...
  async function run() {
    ...
    try {
      const r = await fetch('/api/followup/run', { method:'POST', headers:{'Content-Type':'application/json'}, body:JSON.stringify({scenario, notice_id:noticeId, review_version:options.review_version, question_id:choice, source_preference:sources, queries:Number(queries), documents:Number(documents), seconds:Number(seconds)}) });
